    ...
    <div>
      ...
      <div className="header no-print">
        ...
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.2)', borderRadius: '12px', marginTop: '20px', padding: '4px', gap: '4px' }}>
          <button onClick={() => setMainTab('routes')} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '8px', background: mainTab === 'routes' ? 'white' : 'transparent', color: mainTab === 'routes' ? 'var(--bravo-purple)' : 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>Lijnen</button>
          <button onClick={() => setMainTab('vehicle')} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '8px', background: mainTab === 'vehicle' ? 'white' : 'transparent', color: mainTab === 'vehicle' ? 'var(--bravo-purple)' : 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>Voertuig</button>
          <button onClick={() => setMainTab('checklist')} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '8px', background: mainTab === 'checklist' ? 'white' : 'transparent', color: mainTab === 'checklist' ? 'var(--bravo-purple)' : 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>Checklists</button>
          <button onClick={() => setMainTab('docs')} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '8px', background: mainTab === 'docs' ? 'white' : 'transparent', color: mainTab === 'docs' ? 'var(--bravo-purple)' : 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>Docs</button>
          <button onClick={() => setMainTab('info')} style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '8px', background: mainTab === 'info' ? 'white' : 'transparent', color: mainTab === 'info' ? 'var(--bravo-purple)' : 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>Info</button>
        </div>
      </div>
      ...
      <div className="container">
        ...
        {mainTab === 'vehicle' && (
          <div className="card">
            ...
            {/* TECHNISCHE GEGEVENS WEERGAVE */}
            <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '15px', marginBottom: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <div><label style={{ fontSize: '0.7rem', color: '#6b7280', display: 'block', textTransform: 'uppercase' }}>Huisnummers</label><span style={{ fontWeight: 'bold', fontSize: '0.9rem', color: 'black' }}>{busTypes.find(b => b.id === activeBus)?.fleet}</span></div>
                <div><label style={{ fontSize: '0.7rem', color: '#6b7280', display: 'block', textTransform: 'uppercase' }}>Wielbasis/Lengte</label><span style={{ fontWeight: 'bold', fontSize: '0.9rem', color: 'black' }}>{busTypes.find(b => b.id === activeBus)?.wheelbase}</span></div>
              </div>
            </div>
            ...
            {vehicleChecklist.map((section, idx) => (
              <div key={idx} style={{ marginBottom: '20px' }}>
                ...
                {section.items.map(item => {
                  const uniqueId = `${activeBus}_${item.id}`;
                  return (
                    <div key={item.id} className="checkbox-item" onClick={() => toggleItem(uniqueId)}>
                      <div className="checkbox-content">
                        <div style={{ width: '22px', height: '22px', borderRadius: '6px', border: completed.includes(uniqueId) ? 'none' : '2px solid #d1d5db', background: completed.includes(uniqueId) ? 'var(--success)' : 'transparent', marginRight: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>{completed.includes(uniqueId) && <CheckCircle2 size={14} />}</div>
                        <span style={{ textDecoration: completed.includes(uniqueId) ? 'line-through' : 'none', color: completed.includes(uniqueId) ? '#9ca3af' : 'inherit', fontSize: '0.9rem' }}>{item.text}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        {mainTab === 'checklist' && (
          <div>
            {initialCategories.map((category) => category.id !== 'routes' && (
        ...
        {mainTab === 'docs' && (
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '10px' }}>
              {importantDocuments && importantDocuments.map((doc) => (<button key={doc.id} onClick={() => setPdfModal(doc)} style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', textAlign: 'left', cursor: 'pointer' }}><FileText size={24} color="var(--bravo-purple)" /><span style={{ fontWeight: '600', fontSize: '0.95rem', color: '#374151' }}>{doc.title}</span></button>))}
            </div>
          </div>
        )}
