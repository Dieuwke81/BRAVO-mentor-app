    ...
    <div className="main-wrapper">
      ...
      <div className="header no-print">
        ...
        <div className="student-box">
          ...
          <div className="row">
            <input type="text" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} placeholder="Naam leerling..." className="student-input" />
            <button onClick={addStudent} className="add-btn"><Plus size={20} /></button>
          </div>
        </div>
        ...
        <div className="main-tabs no-scrollbar">
          {['routes', 'vehicle', 'checklist', 'docs', 'info'].map(t => (
            <button key={t} onClick={() => setMainTab(t)} className={mainTab === t ? 'active' : ''}>
              {t === 'routes' ? 'Lijnen' : t === 'vehicle' ? 'Voertuig' : t === 'checklist' ? 'Checklists' : t === 'docs' ? 'Docs' : 'Info'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="container no-print">
              ...
              <div key={item.id} className="item-row">
                ...
                <div className="tally-row">
                  <div className="tally-box">
                    ...
                    <button onClick={() => updateTally(item.id, 'm', 1)}><Plus size={16} /></button>
                  </div>
                  <div className="tally-box green">
                    <button onClick={() => updateTally(item.id, 'z', -1)}><Minus size={16} /></button>
        ...
        {mainTab === 'vehicle' && (
          <div className="card">
            ...
            <div className="sub-tabs no-scrollbar">
              {busTypes.map(bus => (<button key={bus.id} onClick={() => setActiveBus(bus.id)} className={activeBus === bus.id ? 'active' : ''}>{bus.label}</button>))}
            </div>
            {currentBusInfo && (
        ...
        {mainTab === 'info' && (
          <div style={{ paddingBottom: '40px' }}>
            <div className="card ziekmelden">
               <div className="alert-head"><ShieldAlert size={20} /> ZIEKMELDEN</div>
