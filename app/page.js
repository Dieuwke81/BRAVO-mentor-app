      ...
      <div className="container no-print">
        ...
        {mainTab === 'info' && (
          <div>
            ...
            <div className="card" style={{ textAlign: 'center' }}><button onClick={() => window.print()} style={{ background: '#10b981', color: 'white', padding: '12px', borderRadius: '10px', border: 'none', fontWeight: 'bold', width: '100%' }}>Rapport maken</button><button onClick={exportData} style={{ marginTop: '10px', background: 'var(--bravo-purple)', color: 'white', padding: '12px', borderRadius: '10px', border: 'none', width: '100%' }}>Exporteer data</button><label style={{ marginTop: '10px', display: 'block', background: 'white', color: 'var(--bravo-purple)', padding: '12px', borderRadius: '10px', border: '2px solid var(--bravo-purple)' }}>Importeer data<input type="file" onChange={importData} style={{ display: 'none' }} /></label></div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {usefulLinks.map((link, i) => (
                  <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', textDecoration: 'none', color: 'black' }}>
                    <span style={{ fontSize: '0.9rem', fontWeight: '500' }}>{link.name}</span>
                    <ExternalLink size={18} color="var(--bravo-purple)" />
                  </a>
                ))}
              </div>
            </div>

            {contactData.map((group, idx) => (<div key={idx} className="card"><h3 style={{ fontSize: '0.9rem', color: 'var(--bravo-purple)' }}>{group.category}</h3>{group.contacts.map((c, i) => (<div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}><span>{c.name}</span><div style={{ display: 'flex', gap: '5px' }}>{c.phone && <a href={`tel:${c.phone}`} style={{ padding: '6px 10px', background: 'white', border: '1px solid var(--bravo-purple)', borderRadius: '8px', textDecoration: 'none', color: 'var(--bravo-purple)', fontWeight: 'bold', fontSize: '0.8rem' }}>{c.phone}</a>}{c.email && <a href={`mailto:${c.email}`} style={{ padding: '6px 10px', background: '#f0f9ff', borderRadius: '8px', textDecoration: 'none', color: '#0369a1', fontWeight: 'bold', fontSize: '0.8rem' }}>Mail</a>}</div></div>))}</div>))}
          </div>
        )}
      </div>
      <style jsx global>{`
