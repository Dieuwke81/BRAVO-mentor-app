  ...
  return (
    <div className="main-wrapper">
        ...
        {mainTab === 'docs' && (
          <>
            ...
            <div className="card">
              ...
              <div className="links-list">{usefulLinks.map((link, i) => (<a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="useful-link-item"><span>{link.name}</span><ExternalLink size={18} /></a>))}</div>
            </div>
          </>
        )}
      ...
      <style jsx global>{`
        ...
        .useful-link-item { display: flex; justify-content: space-between; align-items: center; padding: 12px; background: var(--bg); border-radius: 10px; text-decoration: none; color: var(--text); border: 1px solid var(--border); margin-bottom: 8px; }
